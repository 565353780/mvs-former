...
class StageNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, tmp=2.0):
        ...
        assert (
            len(src_feats) == len(proj_matrices) - 1
        ...
        with autocast(enabled=False):
            for src_feat, src_proj in zip(src_feats, src_projs):
                ...
                src_proj_new[:, :3, :4] = torch.matmul(
                    src_proj[:, 1, :3, :3], src_proj[:, 0, :3, :4]
                )
                ref_proj_new = ref_proj[:, 0].clone()
                ref_proj_new[:, :3, :4] = torch.matmul(
                    ref_proj[:, 1, :3, :3], ref_proj[:, 0, :3, :4]
                )
                warped_volume, proj_mask = homo_warping_3D_with_mask(
                    src_feat, src_proj_new, ref_proj_new, depth_values
                )
                ...
                if not self.training:
                    similarity = F.normalize(ref_volume, dim=1) * F.normalize(
                        warped_volume, dim=1
                    )
                ...
                if self.fusion_type == "cnn":
                    ...
                    entropy = (-sim_vol_norm * torch.log(sim_vol_norm + 1e-7)).sum(
                        dim=1, keepdim=True
                    )
                    vis_weight = self.vis(entropy)
                elif self.fusion_type == "epipole":
                    vis_weight = torch.softmax(
                        in_prod_vol.sum(1) / self.attn_temp, dim=1
                    ) / math.sqrt(C)  # B D H W
                elif self.fusion_type == "epipoleV2":
                    attn_score = in_prod_vol.sum(1) / torch.clamp(
                        self.attn_temp, 0.1, 10.0
                    )  # [B,D,H,W]
                    attn_score = attn_score + (-10000.0 * proj_mask)
                    vis_weight = torch.softmax(attn_score, dim=1) / math.sqrt(
                        G
                    )  # B D H W
                else:
                    raise NotImplementedError
            ...
            volume_mean = volume_sum / (
                vis_sum.unsqueeze(1) + 1e-6
            )  # volume_sum / (num_views - 1)
        ...
        if self.args["depth_type"] == "ce" or self.args["depth_type"] == "was":
            ...
            if self.training:
                ...
                # vanilla argmax
                depth = torch.gather(
                    depth_values, dim=1, index=idx.unsqueeze(1)
                ).squeeze(1)
            else:
                # regression (t)
                depth = depth_regression(
                    F.softmax(prob_volume_pre * tmp, dim=1), depth_values=depth_values
                )
            # conf
            ...
            mixup_depth = (
                depth_values[:, :-1] * prob_left_normed
                + depth_values[:, 1:] * prob_right_normed
            )  # [B,D-1,H,W]
...
class DINOMVSNet(nn.Module):
    def __init__(self, args):
        ...
        self.vit = vits.__dict__[self.vit_args["vit_arch"]](
            patch_size=self.vit_args["patch_size"], qk_scale=self.vit_args["qk_scale"]
        )
        if os.path.exists(self.vit_args["vit_path"]):
            ...
            print(
                "!!!No weight in",
                self.vit_args["vit_path"],
                "testing should neglect this.",
            )
        ...
        self.fusions = nn.ModuleList(
            [StageNet(args, self.ndepths[i], i) for i in range(len(self.ndepths))]
        )
    ...
    def forward(self, imgs, proj_matrices, depth_values, tmp=2.0):
        ...
        if self.training:
            ...
            vit_h, vit_w = (
                int(H * self.vit_args["rescale"]),
                int(W * self.vit_args["rescale"]),
            )
            vit_imgs = F.interpolate(
                imgs, (vit_h, vit_w), mode="bicubic", align_corners=Align_Corners_Range
            )
            if self.args["fix"]:
                with torch.no_grad():
            ...
            vit_feat = (
                ...
                .reshape(
                    ...
                    self.vit_args["vit_ch"],
                )
                .permute(0, 3, 1, 2)
                .contiguous()
            )  # [BV,C,h,w]
            vit_att = vit_att[:, :, 0, 1:].reshape(
                B * V,
                -1,
                vit_h // self.vit_args["patch_size"],
                vit_w // self.vit_args["patch_size"],
            ...
            if self.multi_scale:
                vit_out, vit_out2, vit_out3 = self.decoder_vit.forward(
                    vit_feat, vit_att
                )
                feat1, feat2, feat3, feat4 = self.decoder.forward(
                    conv01, conv11, conv21, conv31, vit_out, vit_out2, vit_out3
                )
            else:
            ...
            features = {
                "stage1": feat1.reshape(
                    B, V, feat1.shape[1], feat1.shape[2], feat1.shape[3]
                ),
                "stage2": feat2.reshape(
                    B, V, feat2.shape[1], feat2.shape[2], feat2.shape[3]
                ),
                "stage3": feat3.reshape(
                    B, V, feat3.shape[1], feat3.shape[2], feat3.shape[3]
                ),
                "stage4": feat4.reshape(
                    B, V, feat4.shape[1], feat4.shape[2], feat4.shape[3]
                ),
            }
        else:
        ...
        for stage_idx in range(len(self.ndepths)):
            ...
            outputs_stage = self.fusions[stage_idx].forward(
                features_stage, proj_matrices_stage, depth_samples, tmp=tmp
            )
            outputs["stage{}".format(stage_idx + 1)] = outputs_stage
            if (
                outputs_stage["photometric_confidence"].shape[1] != prob_maps.shape[1]
                or outputs_stage["photometric_confidence"].shape[2]
                != prob_maps.shape[2]
            ):
                outputs_stage["photometric_confidence"] = F.interpolate(
                    outputs_stage["photometric_confidence"].unsqueeze(1),
                    [prob_maps.shape[1], prob_maps.shape[2]],
                    mode="nearest",
                ).squeeze(1)
...
class TwinMVSNet(nn.Module):
    ...
    def forward(self, imgs, proj_matrices, depth_values, tmp=2.0):
        ...
        for stage_idx in range(len(self.ndepths)):
            ...
            if self.args["depth_type"] in ["ce", "mixup_ce"]:
                if (
                    outputs_stage["photometric_confidence"].shape[1]
                    != prob_maps.shape[1]
                    or outputs_stage["photometric_confidence"].shape[2]
                    != prob_maps.shape[2]
                ):
