...
def idx_img2cam(idx_img_homo, depth, cam):  # nhw31, n1hw -> nhw41
    ...
    idx_cam = (
        idx_cam / (idx_cam[..., -1:, :] + 1e-9) * depth.permute(0, 2, 3, 1).unsqueeze(4)
    )  # nhw31
    idx_cam_homo = torch.cat(
        [idx_cam, torch.ones_like(idx_cam[..., -1:, :])], dim=-2
    )  # nhw41
...
def project_img(
    src_img, dst_depth, src_cam, dst_cam, height=None, width=None
):  # nchw, n1hw -> nchw, n1hw
    if height is None:
        height = src_img.size()[-2]
    if width is None:
        width = src_img.size()[-1]
    ...
    warped_img = F.grid_sample(
        src_img, warp_coord, mode="bilinear", padding_mode="zeros", align_corners=True
    )
...
def get_reproj(ref_depth, srcs_depth, ref_cam, srcs_cam):  # n1hw, nv1hw -> n1hw
    ...
    srcs2ref_xyd = torch.cat(
        [srcs2ref_idx_img[..., :2, 0], srcs2ref_idx_cam[..., 2:3, 0]], dim=-1
    ).permute(0, 3, 1, 2)  # N3hw

    reproj_xyd_f, in_range_f = project_img(
        srcs2ref_xyd, ref_depth_r, srcs_cam_f, ref_cam_r
    )  # N3hw, N1hw
...
def vis_filter(ref_depth, reproj_xyd, in_range, img_dist_thresh, depth_thresh, vthresh):
    ...
    dist_masks = (reproj_xyd[:, :, :2, :, :] - xy).norm(
        dim=2, keepdim=True
    ) < img_dist_thresh  # nv1hw
    depth_masks = (ref_depth.unsqueeze(1) - reproj_xyd[:, :, 2:, :, :]).abs() < (
        torch.max(ref_depth.unsqueeze(1), reproj_xyd[:, :, 2:, :, :]) * depth_thresh
    )  # nv1hw
    masks = bin_op_reduce(
        [in_range, dist_masks.to(ref_depth.dtype), depth_masks.to(ref_depth.dtype)],
        torch.min,
    )  # nv1hw
...
def ave_fusion(ref_depth, reproj_xyd, masks):
    ave = ((reproj_xyd[:, :, 2:, :, :] * masks).sum(dim=1) + ref_depth) / (
        masks.sum(dim=1) + 1
    )  # n1hw
    return ave
...
def get_reproj_dynamic(ref_depth, srcs_depth, ref_cam, srcs_cam):  # n1hw, nv1hw -> n1hw
    ...
    ref_idx_cam = idx_img2cam(
        idx_img, ref_depth_f, ref_cam_r
    )  # Nhw41     k^-1  [x,y,1] * d
    ref_idx_world = idx_cam2world(
        ref_idx_cam, ref_cam_r
    )  # Nhw41       (R-1)  k^-1  [x,y,1] * d  / [:,:,-1]
    ref2src_idx_cam = idx_world2cam(
        ref_idx_world, srcs_cam_f
    )  # Nhw41   R*  [(R-1)  k^-1  [x,y,1] * d  / [:,:,-1] ]  / [:,:,-1]
    ref2src_idx_img = idx_cam2img(
        ref2src_idx_cam, srcs_cam_f
    )  # Nhw31   K * R*  [(R-1)  k^-1  [x,y,1] * d  / [:,:,-1] ]  / [:,:,-1]
    ...
    warped_src_depth = F.grid_sample(
        srcs_depth_f, proj_xy, mode="bilinear", padding_mode="zeros", align_corners=True
    )

    warp_homo_coord = torch.cat(
        [warp_coord, torch.ones_like(warp_coord[..., -1:])], dim=-1
    ).unsqueeze(-1)  # [n,h,w,3]

    src_idx_cam = idx_img2cam(
        warp_homo_coord, warped_src_depth, srcs_cam_f
    )  # Nhw41     k^-1  [x,y,1] * d
    ...
    # bn  3 h w
    reproj_xyd_f = torch.cat(
        [src2ref_idx_corrd[..., :2, 0], reproj_depth.unsqueeze(-1)], dim=-1
    ).permute(0, 3, 1, 2)
...
def vis_filter_dynamic(ref_depth, reproj_xyd, dist_base=4, rel_diff_base=1300):
    ...
    depth_diff = (
        ref_depth.unsqueeze(1) - reproj_xyd[:, :, 2:, :, :]
    ).abs() / ref_depth.unsqueeze(1)  # nv1hw

    dist_thred = (
        torch.arange(2, v + 1).reshape(1, 1, -1, 1, 1).repeat(n, v, 1, 1, 1).to(device)
        / dist_base
    )
    relative_dist_thred = (
        torch.arange(2, v + 1).reshape(1, 1, -1, 1, 1).repeat(n, v, 1, 1, 1).to(device)
    ...
    masks = torch.min(
        corrd_diff < dist_thred, depth_diff < relative_dist_thred
    )  # [n,v,v-1, h,w]
