...
class Trainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.random_mask = self.config["data_loader"][0]["args"].get(
            "random_mask", False
        )
        self.scale_batch_map = self.config["data_loader"][0]["args"][
            "multi_scale_args"
        ]["scale_batch_map"]
            ...
            for batch_idx, sample in enumerate(dl):
                ...
                for bi in range(iters_to_accumulate):
                    ...
                    if self.depth_type == "re":
                        loss_dict = reg_loss_stage4(
                            outputs,
                            depth_gt_ms_tmp,
                            mask_ms_tmp,
                            dlossw=[1, 1, 1, 1],
                            depth_interval=depth_interval[b_start:b_end],
                            mask_out_range=self.mask_out_range,
                            inverse_depth=self.inverse_depth,
                        )
                    elif self.depth_type == "was":
                        loss_dict = wasserstein_loss(
                            outputs,
                            depth_gt_ms_tmp,
                            mask_ms_tmp,
                            dlossw=[1, 1, 1, 1],
                        ...
                        loss_dict = mixup_ce_loss_stage4(
                            outputs,
                            depth_gt_ms_tmp,
                            mask_ms_tmp,
                            dlossw=[1, 1, 1, 1],
                    ...
                    for key in loss_dict:
                        ...
                        total_loss_dict[key] = (
                            total_loss_dict[key] + loss_dict[key] / iters_to_accumulate
                        )
                    ...
                    with torch.no_grad():
                        for group in self.optimizer.param_groups:
                            for param in group["params"]:
                                ...
                                scaled_grads[group["layer_name"]].append(
                                    v.item() / self.scaler.get_scale()
                                )
                ...
                if batch_idx % self.log_step == 0 and self.rank == 0:
                    ...
                    print_str += ", size:{}x{}, bs:{}".format(
                        imgs.shape[3], imgs.shape[4], bs
                    )
                    if self.fp16:
    ...
    def _valid_epoch(self, epoch):
            ...
            for dl in self.valid_data_loader:
                ...
                for batch_idx, sample in enumerate(tqdm(dl)):
                    ...
                    if self.config["data_loader"][0]["type"] == "BlendedLoader":
                        ...
                        scalar_outputs = {
                            "abs_depth_error": AbsDepthError_metrics(
                                depth_est, depth_gt, mask > 0.5
                            ),
                            "thres2mm_error": Thres_metrics(
                                depth_est, depth_gt, mask > 0.5, di * 2
                            ),
                            "thres4mm_error": Thres_metrics(
                                depth_est, depth_gt, mask > 0.5, di * 4
                            ),
                            "thres8mm_error": Thres_metrics(
                                depth_est, depth_gt, mask > 0.5, di * 8
                            ),
                            "thres14mm_error": Thres_metrics(
                                depth_est, depth_gt, mask > 0.5, di * 14
                            ),
                        }

                    scalar_outputs = tensor2float(scalar_outputs)
                    image_outputs = {
                        "pred_depth": outputs["refined_depth"] * mask,
                        "gt_depth": depth_gt,
                        "ref_img": imgs[:, 0],
                    }
                ...
                val_metrics["mean_error"] = (
                    val_metrics["thres2mm_error"]
                    + val_metrics["thres4mm_error"]
                    + val_metrics["thres8mm_error"]
                    + val_metrics["thres14mm_error"]
                )
        ...
        for k in val_metrics:
            val_metrics[k] = torch.tensor(
                val_metrics[k], device=self.rank, dtype=torch.float32
            )
        self.model.train()
