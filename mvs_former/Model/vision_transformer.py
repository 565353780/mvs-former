...
from mvs_former.Method.utils import trunc_normal_
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x):  # x:[BV,HW,C]
        ...
        qkv = (
            self.qkv(x)
            .reshape(B, V * HW, 3, self.num_heads, C // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )
...
class CrossBlock(nn.Module):
    def __init__(
        ...
        self.attn = CrossAttention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, nview=nview
        )
        # self.norm1 = norm_layer(dim)
        # self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        ...
        self.rel_epipole_emb = PositionEmbeddingSine(
            dim // 4, scale=32 * math.pi
        )  # 每个patch相对方向大致变化在0.01~0.05
        self.abs_epipole_dis_emb = SinglePositionEmbeddingSine(
            dim // 4, scale=2 * math.pi
        )  # 每个极点距离，大致在100~500
        self.abs_epipole_dir_emb = PositionEmbeddingSine(
            dim // 8, scale=2 * math.pi
        )  # 每个极点方向
    ...
    def forward(self, x, epipole, height, width):
        ...
        if epipole is None:
            ...
            src_ids = torch.ones(
                (B, self.nview - 1, HW), dtype=torch.long, device=x.device
            )
            ...
            y_, x_ = torch.meshgrid(
                [
                    torch.arange(0, height, dtype=torch.float32, device=x.device),
                    torch.arange(0, width, dtype=torch.float32, device=x.device),
                ]
            )
            x_, y_ = x_.contiguous(), y_.contiguous()
            x_ = x_.reshape(1, 1, height, width)
            y_ = y_.reshape(1, 1, height, width)
            epipole_map = epipole.reshape(
                B, self.nview - 1, 2, 1, 1
            )  # [B, V-1, 2, 1, 1]
            ...
            epipole_dis = (
                torch.sqrt(epipole[:, :, 0] ** 2 + epipole[:, :, 1] ** 2) / 512
            )
            epipole_dis = torch.clamp(epipole_dis, 0, 1.0)
            epipole_dis = epipole_dis.reshape(B, self.nview - 1, 1, 1).repeat(
                1, 1, height, width
            )  # [B,V-1,H,W]
            epipole_dis = epipole_dis.reshape(
                B * (self.nview - 1), HW - 1
            )  # [B(V-1),HW](0~1)
            ...
            ref_ids = torch.zeros(
                (B, HW), dtype=torch.long, device=x.device
            )  # [B,1+HW]
            sep_ids = torch.ones(
                (B, self.nview - 1), dtype=torch.long, device=x.device
            )  # [B,V-1]
            ref_emb = self.tok_embeddings(ref_ids).unsqueeze(1)  # [B,1,1+HW,C]
            sep_emb = self.tok_embeddings(sep_ids).unsqueeze(2)  # [B,V-1,1,C]
            src_emb = torch.cat([sep_emb, epi_emb], dim=2)  # [B,V-1,1+HW,C]
    ...
    def forward(self, x, src_epipoles=None):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if self.cross_att and (i + 1) % self.cross_inter == 0:
                x = self.cross_blocks[(i + 1) // self.cross_inter - 1](
                    x, src_epipoles, self.height, self.width
                )
        x = self.norm(x)
        return x
