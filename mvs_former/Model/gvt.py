...
class GroupAttention(nn.Module):
    ...
    def forward_mask(self, x, H, W):
        ...
        x = x.reshape(B, _h, self.ws, _w, self.ws, C).transpose(
            2, 3
        )  # B, _h, _w, ws, ws, C
        mask = (
            mask.reshape(1, _h, self.ws, _w, self.ws)
            .transpose(2, 3)
            .reshape(1, _h * _w, self.ws * self.ws)
        )
