...
class MVSDataset(Dataset):
    ...
    def read_mask_hr(self, filename):
        ...
        np_img = cv2.resize(
            np_img, (self.max_w, self.max_h), interpolation=cv2.INTER_NEAREST
        )

        np_img_ms = {"stage4": np_img}
        return np_img_ms
    ...
    def read_depth_hr(self, filename):
        ...
        depth_lr = cv2.resize(
            depth_hr, (self.max_w, self.max_h), interpolation=cv2.INTER_NEAREST
        )

        depth_lr_ms = {"stage4": depth_lr}
        return depth_lr_ms
        ...
        for i, vid in enumerate(view_ids):
            ...
            if self.kwargs["dataset"] == "tt":
                if self.kwargs["use_short_range"]:
                    proj_mat_filename = os.path.join(
                        self.datapath,
                        "short_range_cameras/cams_{}/{:0>8}_cam.txt".format(
                            scan.lower(), vid
                        ),
                    )
                else:
                ...
                proj_mat_filename = os.path.join(
                    self.datapath, "{}/cams_1/{:0>8}_cam.txt".format(scan, vid)
                )
            ...
            if i == 0:  # reference view
                if self.kwargs["dataset"] == "dtu":
                    mask_read_ms = None
                    if os.path.exists(mask_filename_hr):
