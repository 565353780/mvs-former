...
from mvs_former.Method.lr_decay import param_groups_lrd
from mvs_former.Method.utils import get_lr_schedule_with_warmup
from mvs_former.Model.mvsformer_model import DINOMVSNet, TwinMVSNet
from mvs_former.Module.trainer import Trainer
...
def main(gpu, args, config):
    ...
    for dl_params in config["data_loader"]:
        ...
        if dl_name == "BlendedLoader":
            train_dataset = BlendedMVSMSDataset(**train_dl_args)
        else:
            train_dataset = DTUMVSMSDataset(**train_dl_args)
        ...
        train_loader = DataLoader(
            ...
            shuffle=False,
            pin_memory=True,
            batch_size=train_dl_args["batch_size"],
            num_workers=train_dl_args["num_workers"],
        ...
        if dl_name == "BlendedLoader":
            val_dataset = BlendedMVSMSDataset(**val_dl_args)
        else:
            val_dataset = DTUMVSMSDataset(**val_dl_args)
    ...
    if config["arch"]["args"]["vit_args"].get("twin", False):
            ...
            param_groups = [
                {
                    "params": [
                        ...
                        if p.requires_grad and n.startswith("vit")
                    ],
                    "lr": opt_args["vit_lr"],
        ...
        param_groups.append(
            {
                "params": [
                    p
                    for n, p in model.named_parameters()
                    if p.requires_grad and not n.startswith("vit")
                ],
                "lr": opt_args["lr"],
                "weight_decay": 0.0,
                "vit_param": False,
        ...
        if config["arch"]["args"].get("fix") is True:
            ...
            param_groups = param_groups_lrd(
                ...
                layer_decay=opt_args["layer_decay"],
            )
        param_groups.append(
            {
                "params": [
    ...
    optimizer = torch.optim.AdamW(
        param_groups, lr=opt_args["lr"], weight_decay=opt_args["weight_decay"]
    )
    lr_scheduler = get_lr_schedule_with_warmup(
        optimizer,
        num_warmup_steps=opt_args["warmup_steps"],
    ...
    if is_finetune:
        ...
        if not reset_sche:
            ...
            for _ in tqdm(
                range(checkpoint["epoch"] * len(train_data_loaders[0])),
    ...
    trainer = Trainer(
        ...
        optimizer,
        config=config,
        data_loader=train_data_loaders,
        ddp=args.DDP,
        valid_data_loader=valid_data_loaders,
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--dtu_model_path", default=None, type=str, help="MVS model trained on DTU"
    )
    ...
    args.add_argument(
        "--debug",
        action="store_true",
        help="slow down the training, but can check fp16 overflow",
    )
